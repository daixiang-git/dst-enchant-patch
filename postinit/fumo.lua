...
-- 读取配置
GLOBAL.HHCompatAPI.UnknownTagEnabled = GetModConfigData("ENABLE_UNKNOWN_TAG") or
                                           false
...
local KNOWN_CATEGORY_TAGS = {
    "pig", "rabbit", "fish", "gear", "spider", "dog", "frog", "insect",
    "monkey", "shadow", "plant",
    string.char(0x63, 0x6f, 0x6d, 0x6d, 0x6f, 0x6e, 0x5f, 0x6d, 0x6f, 0x6e,
                0x73, 0x74, 0x65, 0x72), -- "common_monster"
    string.char(0x65, 0x6c, 0x69, 0x74, 0x65, 0x5f, 0x6d, 0x6f, 0x6e, 0x73,
                0x74, 0x65, 0x72), -- "elite_monster"
    string.char(0x62, 0x6f, 0x73, 0x73, 0x5f, 0x6d, 0x6f, 0x6e, 0x73, 0x74,
                0x65, 0x72) -- "boss_monster"
}

local boss_tag = string.char(0x62, 0x6f, 0x73, 0x73, 0x5f, 0x6d, 0x6f, 0x6e,
                             0x73, 0x74, 0x65, 0x72)
local elite_tag = string.char(0x65, 0x6c, 0x69, 0x74, 0x65, 0x5f, 0x6d, 0x6f,
                              0x6e, 0x73, 0x74, 0x65, 0x72)
local common_tag = string.char(0x63, 0x6f, 0x6d, 0x6d, 0x6f, 0x6e, 0x5f, 0x6d,
                               0x6f, 0x6e, 0x73, 0x74, 0x65, 0x72) -- "common_monster"

-- 加载兼容配置文件
local compat_config = GLOBAL.require("compat/compat_config")
local compat_modules = compat_config.modules
-- 黑名单不参与附魔
local blacklist = {["siving_foenix"] = true, ["siving_moenix"] = true}
...
for _, mod in ipairs(compat_modules) do
    ...
    if not status then print("[HH兼容] 加载错误:", mod.name, "->", err) end
end

-- 判断是否为生物的函数
local function IsCreature(inst)
    if inst and inst.prefab == "abigail" then return true end
    return inst.components.health ~= nil and inst.components.combat ~= nil and
               inst.components.lootdropper ~= nil
end
...
local function RegisterCustomMonsters()
    ...
    if GLOBAL.LeifEnabled then
        custom_mobs["leif"] = {[common_tag] = "普通"}
        custom_mobs["leif_sparse"] = {[common_tag] = "普通"}
    end

    if GLOBAL.WormEnabled then
        custom_mobs["worm"] = {[common_tag] = "普通"}
        custom_mobs["yots_worm"] = {[common_tag] = "普通"}
    end
    custom_mobs["prime_mate_p"] = {[common_tag] = "普通"}

    if next(custom_mobs) == nil then return end
...
AddPrefabPostInitAny(function(inst)
    if not inst or not inst.prefab or not GLOBAL.HHCompatAPI.UnknownTagEnabled or
        not IsCreature(inst) or IsPlayer(inst) then return end

    -- 黑名单
    if blacklist[inst.prefab] then return end
...
function UpvalueHacker.GetUpvalue(fn, ...)
    ...
    for j, var in ipairs({...}) do
        assert(type(fn) == "function",
               "We were looking for " .. var .. ", but the value before it, " ..
                   prv_var .. ", wasn't a function (it was a " .. type(fn) ..
                   "). Here's the full chain: " ..
                   table.concat({"(the starting point)", ...}, ", "))
        prv = fn
        prv_var = var
...
local function CheckAttackRelationship(inst, target)
    ...
    if instLeader ~= nil then return true end

    local targetLeader = target.components and target.components.follower and
                             target.components.follower.leader
    if targetLeader ~= nil then return true end
...
local function BothHaveCategoryTags(inst, target)
    if not inst.hh_tags or not target.hh_tags then return false end
    -- 增强关系检测优先
    ...
    for tag, _ in pairs(target.hh_tags) do
        ...
        if hasTargetTags then break end
    end
...
local function SpawnFx(inst)
    if inst:IsValid() then
        SpawnPrefab("stalker_shield").Transform:SetPosition(
